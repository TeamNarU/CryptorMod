package com.cryptorinc.cryptormod;
...
public class CryptorModVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "cryptormod_mapvars";
		public MapVariables() {
			super(DATA_NAME);
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				CryptorMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				CryptorMod.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "cryptormod_worldvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				CryptorMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				CryptorMod.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				if (message.type == 0)
					CryptorMod.PACKET_HANDLER.sendToAll(message);
				else
					CryptorMod.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
