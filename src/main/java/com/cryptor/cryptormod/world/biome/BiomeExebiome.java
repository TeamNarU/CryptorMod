...
public class BiomeExebiome extends ElementsCryptorMod.ModElement {
	@GameRegistry.ObjectHolder("cryptormod:exebiome")
	public static final BiomeGenCustom biome = null;
	public BiomeExebiome(ElementsCryptorMod instance) {
		super(instance, 26);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			...
			topBlock = BlockExeblock.block.getDefaultState();
			fillerBlock = BlockExeblock.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockExeblock.block.getDefaultState().getBlock() || ground == BlockExeblock.block.getDefaultState().getBlock())
							&& (ground2 == BlockExeblock.block.getDefaultState().getBlock()
									|| ground2 == BlockExeblock.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockExeblock.block.getDefaultState(), 2);
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == BlockExeblock.block.getDefaultState().getBlock()
					|| blockType == BlockExeblock.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockExeblock.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockExeblock.block.getDefaultState());
		}
